...
void Visualisation::loop(Vec_2d<Particle> result, Vec_2d<double> density, double h) {
    ...
    double rhos_step = rhos_range / 320;

    double rhos_quarter = min_rho + 320 * rhos_step / 4;
    double rhos_half = rhos_quarter + 160 * rhos_step / 2;
                ...
                if (rho < rhos_quarter) {
                    for (int k = 1; k < 320; k++) {
                        if (min_rho + (k - 1) * rhos_step / 4 <= rho && rho < min_rho + k * rhos_step / 4) {
                            int color = k / 4;
                            sprites[i].setFillColor(hsv(color, 1.f, 0.5));
                            sprites[i].setOutlineColor(hsv(color, 1.f, 1.f));
                    ...
                    for (int k = 1; k < 160; k++) {
                        if (rhos_quarter + (k - 1) * rhos_step / 2 <= rho && rho < rhos_quarter + k * rhos_step / 2) {
                            ...
                            sprites[i].setFillColor(hsv(color, 1.f, 0.5));
                            sprites[i].setOutlineColor(hsv(color, 1.f, 1.f));
                            //std::cout <<"I am here2 "<< rho << " " << color << std::endl;
                        }
                    }
                }
                    ...
                    for (int k = 1; k < 160; k++) {
                        ...
                        if (rhos_half + (k - 1) * rhos_step <= rho && rho < rhos_half + k * rhos_step) {
                            sprites[i].setFillColor(hsv(color, 1.f, 0.5));
                            sprites[i].setOutlineColor(hsv(color, 1.f, 1.f));
                            //std::cout <<"I am here3 "<< rho << " " << color<< std::endl;
                        }
                    }
