...
void calc_density(Vec_1d<Particle>& particles, Vec_1d<Particle>* grid, float h, int N_grid_cells, Iterator begin, Iterator end) {
	int N = end - begin;
	//std::cout << (*begin).get_position()[0]<< std::endl;
   // std::cout<< "aiaiaiaia "<<static_cast<int>(begin - particles.begin())<< " aiaiaiaia "<< std::endl;
    //std::cout<< "aiaiaiaia "<<static_cast<int>(begin - particles.begin())<< " aiaiaiaia "<< std::endl;
	for(int i = (begin - particles.begin()); i < N + (begin - particles.begin()); i++) {
		particles[i].change_density(particles[i].get_m() * particles[i].W(particles[i], h));
		//std::cout << particles[i].get_position()[0];
		...
		for (int k = m_grid - 1; k < m_grid + 2; k++) {
          //  std::cout << "calculation rhos" << std::endl;
...
void calc_density_for_all(Vec_1d<Particle>& particles, Vec_1d<Particle>* grid, float h, int N_grid_cells){
    ...
    auto block_size = N / num_threads;
    Vec_1d<std::thread> threads (num_threads);
    std::thread other1(calc_density<decltype(begin)>, std::ref(particles), std::ref(grid),  h, N_grid_cells,  begin, begin + block_size * 1);
    std::thread other2(calc_density<decltype(begin)>, std::ref(particles), std::ref(grid),  h, N_grid_cells,  begin + block_size * 1, begin + block_size * 2);
    std::thread other3(calc_density<decltype(begin)>, std::ref(particles), std::ref(grid),  h, N_grid_cells,  begin + block_size * 2, begin + block_size * 3);
  //  std::cout << "thread1" << std::endl;
    calc_density<decltype(begin)>(std::ref(particles), std::ref(grid),  h, N_grid_cells,  begin + block_size * 3, end);
...
void calc_density_for_all1(Vec_1d<Particle>& particles, Vec_1d<Particle>* grid, float h, int N_grid_cells) {
...
Vec_2d<float> acceleration1(Vec_1d<Particle> particles, Vec_1d<Particle>* grid, float h, float k, float n, float lmbda, float nu, int N_grid_cells) {
    int N = particles.size();
    Vec_2d<float> a(N, Vec_1d<float>(2));
    calc_density_for_all1(particles, grid, h, N_grid_cells);
    calc_pressure_for_all(particles, k, n);
    Vec_1d<float> a_p(2);
    for (int i = 0; i < N; i++) {
        a[i][0] += - nu * particles[i].get_velocity()[0] - lmbda * particles[i].get_position()[0];
...
Vec_2d<Particle> calc1(Vec_1d<Particle>& particles, float h, float d, float k, float n, float lmbda, float nu, float tmax, float dt){
    int N = particles.size();
    int Nt = tmax / dt;
    Vec_2d<Particle> result (Nt);
    ...
    Vec_2d<float> acc = acceleration1(particles, grid, h, k, n, lmbda, nu, N_grid_cells);
    Vec_1d<float> rho(N);
    for (int i = 1; i < Nt; i ++){
        for (int j = 0; j < N; j++){
            particles[j].change_velocity(acc[j], dt);
            particles[j].change_position(particles[j].get_velocity(), dt);
            acc = acceleration1(particles, grid, h, k, n, lmbda, nu, N_grid_cells);
            particles[j].change_velocity(acc[j], dt);
        }
